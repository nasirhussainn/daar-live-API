...
const {
  ...
  getChatHeadersByReferenceId,
  getChatDetailsById,
  getUnreadChatCount,
} = require("../../controller/chat/chatController");
const {
  sendAdminDirectMessage1,
} = require("../../controller/chat/chatAdminController");
const {
  sendAdminDirectMessage,
  getAdminChatsByParticipant,
  getChatByIdForUser,
  getChatByIdForAdmin,
} = require("../../controller/chat/adminChatController");
...
module.exports = (io) => {
  ...
  router.post("/send", upload.single("media"), (req, res, next) =>
    sendMessage(req, res, next, io),
  );
  router.post("/admin", upload.single("media"), (req, res, next) =>
    sendAdminDirectMessage1(req, res, next, io),
  );
  ...
  // -----------------------------------------------------------------------------
  // Admin Chat
  router.post("/admin/send", upload.single("media"), (req, res, next) =>
    sendAdminDirectMessage(req, res, next, io),
  );
