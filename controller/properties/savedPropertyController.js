...
const {
  getReviewsWithCount,
  getReviewCount,
} = require("../../controller/reviews/getReviewsWithCount");
  ...
  try {
    ...
    if (savedProperty) {
      if (savedProperty.status === "like") {
        return res
          .status(200)
          .json({ message: "Property already liked.", savedProperty });
      }
      ...
      return res
        .status(200)
        .json({ message: "Property liked successfully.", savedProperty });
    }
    ...
    res
      .status(201)
      .json({ message: "Property liked (saved) successfully.", savedProperty });
  } catch (error) {
...
exports.dislikeProperty = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Property disliked (removed) successfully." });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
