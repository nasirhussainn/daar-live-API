...
const getRealtorStats = async (req, res) => {
  try {
    ...
    const [currentListed, allListed, soldProperties, rentedData] = await Promise.all([
      // Count properties listed in the last month
      Property.countDocuments({ owner_id: realtorId, created_at: { $gte: oneMonthAgo } }),
      ...
      Property.aggregate([
        ...
        { $group: { _id: null, totalSoldRevenue: { $sum: { $toDouble: "$price" } } } },
      ]),

      // Find rented bookings where the property is for rent and sum revenue
      Booking.aggregate([
        { $match: { realtor_id: realtorId, status: "completed", booking_type: "property" } },
        {
          $lookup: {
            from: "properties",
    ...
    const totalSoldRevenue = soldProperties.length > 0 ? soldProperties[0].totalSoldRevenue : 0;
