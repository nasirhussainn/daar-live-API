...
exports.updatePlan = async (req, res) => {
    try {
      ...
      if (req.body.planName) {
        ...
        if (existingPlan) {
          return res.status(400).json({ message: "planName must be unique. This name is already used." });
        }
  
        updateData.planName = req.body.planName;
      }
  
      if (req.body.noOfPropertyListing) updateData.noOfPropertyListing = req.body.noOfPropertyListing;
      if (req.body.noOfEventListing) updateData.noOfEventListing = req.body.noOfEventListing;
      if (req.body.planAmount) updateData.planAmount = req.body.planAmount;
  
      if (req.body.planDescription) {
        const translatedDescription = await translateText(req.body.planDescription);
        updateData.planDescription = translatedDescription;
      }
  
      const plan = await SubscriptionPlan.findByIdAndUpdate(plan_id, updateData, { new: true });
