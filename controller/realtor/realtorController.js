...
exports.addBankDetails = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Bank details added successfully", data: realtor });
  } catch (error) {
...
exports.deleteBankDetailById = async (req, res) => {
  try {
    ...
    if (!realtor) {
      return res
        .status(404)
        .json({ message: "Realtor or bank detail not found" });
    }
...
exports.getAllBankDetails = async (req, res) => {
  try {
    const realtors = await Realtor.find({
      "bank_details.account_holder_name": { $exists: true },
...
exports.updateBankDetails = async (req, res) => {
  try {
    const { bank_id } = req.params; // Get from URL parameters
    const { bank_name, account_holder_name, account_number, branch_name } =
      req.body;
    ...
    if (bank_name !== undefined)
      updateQuery["bank_details.$.bank_name"] = bank_name;
    if (account_holder_name !== undefined)
      updateQuery["bank_details.$.account_holder_name"] = account_holder_name;
    if (account_number !== undefined)
      updateQuery["bank_details.$.account_number"] = account_number;
    if (branch_name !== undefined)
      updateQuery["bank_details.$.branch_name"] = branch_name;
    ...
    res
      .status(200)
      .json({ message: "Bank details updated successfully", data: realtor });
  } catch (error) {
    console.error("Error updating bank details:", error);
    res.status(500).json({ message: "Server error", error: error.message });
  }
